...
export default function App() {
  ...
  const [targetFrameLayout, setTargetFrameLayout] = useState(null);
  ...
  // Preprocess a base64 image or tensor into [1, H, W, 1] float32 tensor in [0,1]
  const preprocessImage = (input, targetH, targetW) => {
    try {
      let img;
      ...
      if (typeof input === 'string') {
        ...
        img = decodeJpeg(bytes, 3); // [H,W,3] uint8
      } else {
        // Handle tensor input (from cropping)
        img = input;
      }
      ...
      // Only dispose if we created the tensor from base64
      if (typeof input === 'string') {
        img.dispose();
      }
  ...
  // Crop image to target frame area before processing
  const cropImageToTargetFrame = (base64) => {
    try {
      const bytes = Uint8Array.from(Buffer.from(base64, 'base64'));
      const img = decodeJpeg(bytes, 3); // [H,W,3] uint8
      
      const result = tf.tidy(() => {
        const [imgHeight, imgWidth] = img.shape;
        
        // Calculate crop coordinates relative to the image
        // The target frame is centered in the camera view (300x200 pixels)
        const frameWidth = Math.min(300, imgWidth * 0.6); // Use up to 60% of image width
        const frameHeight = Math.min(200, imgHeight * 0.4); // Use up to 40% of image height
        
        // Calculate the center of the camera view
        const centerX = imgWidth / 2;
        const centerY = imgHeight / 2;
        
        // Calculate crop bounds (ensure they're within image bounds)
        const cropLeft = Math.max(0, Math.floor(centerX - frameWidth / 2));
        const cropTop = Math.max(0, Math.floor(centerY - frameHeight / 2));
        const cropRight = Math.min(imgWidth, Math.floor(centerX + frameWidth / 2));
        const cropBottom = Math.min(imgHeight, Math.floor(centerY + frameHeight / 2));
        
        // Calculate actual crop dimensions
        const cropWidth = cropRight - cropLeft;
        const cropHeight = cropBottom - cropTop;
        
        console.log(`Cropping image from ${imgWidth}x${imgHeight} to ${cropWidth}x${cropHeight} at (${cropLeft}, ${cropTop})`);
        
        // Perform the crop using tf.slice
        const cropped = tf.slice(img, [cropTop, cropLeft, 0], [cropHeight, cropWidth, 3]);
      ...
      console.warn('Failed to crop image, using full image:', e?.message);
      // Return null to indicate we should fall back to the original base64
      return null;
    }
  };
  ...
  // Real TensorFlow.js model prediction (supports layers and graph)
      if (model.type?.startsWith('tfjs') && model.model) {
        ...
        // First try to crop the image to the target frame area
        console.log('Cropping image to target frame area...');
        const croppedImage = cropImageToTargetFrame(imageBase64);
        
        // Then preprocess the image (cropped tensor or fallback to original base64)
        if (croppedImage) {
          // Use cropped tensor
          inputData = preprocessImage(croppedImage, inputShape[1], inputShape[2]);
          // Clean up the cropped tensor
          croppedImage.dispose();
        } else {
          // Fallback to original base64 image
          console.log('Using full image as fallback');
          inputData = preprocessImage(imageBase64, inputShape[1], inputShape[2]);
        }
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.overlay}>
        ...
        <View 
          style={styles.targetFrame}
          onLayout={(event) => {
            const { x, y, width, height } = event.nativeEvent.layout;
            setTargetFrameLayout({ x, y, width, height });
            console.log('Target frame layout:', { x, y, width, height });
          }}
        >
          <Text style={styles.overlayText}>
            {!cameraReady ? 'Starting camera...' :
             !model ? 'Loading model...' :
             isProcessing ? 'Processing cropped region...' :
